...
const Settings = (() => {
  ...
  function load() {
    ...
    setVal("cfgApiBase", cfg.apiBase || "https://wp-bot-backend-89il.onrender.com");
    const base = cfg.apiBase || "https://wp-bot-backend-89il.onrender.com";
  ...
  function saveTwilio() {
    const cfg = {
      ...
      apiBase: getVal("cfgApiBase") || "https://wp-bot-backend-89il.onrender.com",
    };
  ...
  function saveWebhook() {
    const cfg = store.get(LS.CONFIG) || DEFAULT_CONFIG;
    cfg.apiBase = getVal("cfgApiBase") || "https://wp-bot-backend-89il.onrender.com";
    store.set(LS.CONFIG, cfg);
    const el = document.getElementById("webhookUrl");
    if (el) el.textContent = cfg.apiBase + "/webhook";
...
const App = (() => {
  ...
  function getApiBase() {
    ...
    return (cfg && cfg.apiBase) ? cfg.apiBase : "https://wp-bot-backend-89il.onrender.com";
  }
  ...
  function init() {
    // ── One-time migration: reset old localhost config to new backend URL ──
    const _cfg = store.get(LS.CONFIG);
    if (!_cfg || _cfg.apiBase === "https://wp-bot-backend-89il.onrender.com" || !_cfg.apiBase) {
      store.set(LS.CONFIG, { ...DEFAULT_CONFIG });
    }
    document.querySelectorAll(".nav-item").forEach(el =>
